...
let total_comment = () => {
    let total = 0; //membuat variable total
    for (let i = 0; i < comments.length; i++) { //mengloop setiap array comments
        total += 1; 
        if (comments[i].replies) { //jika array comments[i].replies tidak kosong maka
            for (let j = 0; j < comments[i].replies.length; j++) { //mengloop setiap array comments[i].replies
                total += 1;
                if (comments[i].replies[j].replies) { //jika array comments[i].replies[j].replies tidak kosong maka
                    for (let k = 0; k < comments[i].replies[j].replies.length; k++) { //mengloop setiap array comments[i].replies[j].replies
                        total += 1;
                    }
                }
            }
